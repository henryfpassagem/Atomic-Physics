...
class IonSim:
    ...
    def init_thermal(self, n, T, sigma_x=1e-6, sigma_y=1e-6, sigma_z=1e-6, min_d=2e-6):
    ...
    def F_U(self, x, y, z, t):
        ...
        Fx = -(self.U(x + _d, y, z, t) - self.U(x - _d, y, z, t)) / (2 * _d)
        Fy = -(self.U(x, y + _d, z, t) - self.U(x, y - _d, z, t)) / (2 * _d)
        Fz = -(self.U(x, y, z + _d, t) - self.U(x, y, z - _d, t)) / (2 * _d)
    ...
    def distances(self):
        ...
        return {'min': np.min(r[np.nonzero(r)]), 'max': np.max(r)}
    ...
    def f(self, t, x):
        ...
        xp = np.zeros(x.shape)
        xp[0:3, :] = x[3:6, :]
        xp[3:6, :] = self.F_total(*x[0:3, :], t, x[3:6, :])
        xp[3, :] /= self.m
        xp[4, :] /= self.m
        xp[5, :] /= self.m
        return xp.ravel()
...
class IonPlot(IonSim):
    ...
    def plot(self, i=[], dim=0):
        ...
        '''
        if not i:
            plt.plot(self.t / 1e-6, self.x[:, dim, :] / 1e-6);
        else:
            plt.plot(self.t / 1e-6, self.x[:, dim, i] / 1e-6);
        plt.xlabel('Time (us)')
    ...
    def animate(self):
        ...
        ax = plt.axes(xlim=(-rmax / 1e-6, rmax / 1e-6), ylim=(-rmax / 1e-6, rmax / 1e-6))
        ax.set_aspect(1.0)
        ...
        def draw_frame(frame_i):
            points.set_data(self.x[frame_i, 0, :] / 1e-6, self.x[frame_i, 1, :] / 1e-6)
            for i, line in enumerate(lines):
                line.set_data(self.x[(frame_i - 5):frame_i, 0, i] / 1e-6,
                              self.x[(frame_i - 5):frame_i, 1, i] / 1e-6)
            return lines
